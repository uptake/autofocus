...
```bash
python train_model.py --image_dir ~/flower_photos
```
...
```bash
python train_model.py (... other args as before ...) \
...
PARAMS_TO_LOG = {'image_dir': 'basic.image_dir',
                 'tfhub_module': 'basic.tfhub_module',
                 'how_many_training_steps': 'hyper.num_train_steps',
                 'train_batch_size': 'hyper.train_batch_size',
                 'learning_rate': 'hyper.learning_rate',
                 'flip_left_right': 'transform.flip_lr',
                 'random_brightness': 'transform.random_brightness',
                 'random_crop': 'transform.random_crop',
                 'random_scale': 'transform.random_scale',
                 'test_batch_size': 'scoring.test_batch_size',
                 'testing_percentage': 'scoring.testing_percentage',
                 'validation_batch_size': 'scoring.validation_batch_size',
                 'validation_percentage': 'scoring.validation_percentage',
                 }
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
  ...
  _log_confusion_matrix(predictions, test_ground_truth, labels)
  _log_misclassified_test_images(test_filenames, predictions, test_ground_truth, image_lists)
...
def _log_confusion_matrix(predictions, test_ground_truth, labels):
    y_true = [labels[truth] for truth in test_ground_truth]
...
def _log_misclassified_test_images(
        test_filenames, predictions, test_ground_truth, image_lists
        ):
    misclassified_test_images = [
        f'{test_filename}, {list(image_lists.keys())[predictions[i]]}'
        for i, test_filename in enumerate(test_filenames)
        if predictions[i] != test_ground_truth[i]
        ]
    with open('misclassified_test_images.csv', 'a') as f:
        f.write('filepath, predicted_label\n')
        for line in misclassified_test_images:
            f.write(line + '\n')
    mlflow.log_artifact('misclassified_test_images.csv')
    os.remove('misclassified_test_images.csv')
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in tqdm(range(FLAGS.how_many_training_steps)):
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='',
        help='Do not parse filenames for locations for metric reporting.')
...
def _log_params():
  ...
  for param_name in PARAMS_TO_LOG:
    mlflow.log_param(PARAMS_TO_LOG[param_name], flags_dict[param_name])
